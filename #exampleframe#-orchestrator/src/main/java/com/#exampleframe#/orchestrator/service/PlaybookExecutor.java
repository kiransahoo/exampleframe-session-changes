...
public class PlaybookExecutor {
    ...
    public Flux<ServerSentEvent<String>> execute(
        ...
        return Mono.fromCallable(() -> executeSteps(playbook, params, threadId, originalQuery, true, deadline))
                .subscribeOn(reactor.core.scheduler.Schedulers.boundedElastic())
                .flatMapMany(results ->
                        Flux.fromIterable(buildTerminalEvents(playbook, results, originalQuery)))
    ...
    public String executeBlocking(
        ...
        List<StepResult> results = executeSteps(playbook, params, threadId, originalQuery, false, deadline);
        ...
        return synthesizeReport(playbook, results, originalQuery);
    }
        ...
        while (i < steps.size()) {
            ...
            if (steps.get(i).parallel() && i + 1 < steps.size() && steps.get(i + 1).parallel()) {
                ...
                List<StepResult> batchResults = executeParallelBatch(
                        batch, context, threadId, playbook.name(), emitProgress, deadline);
                ...
                StepResult result = executeSingleStep(
                        step, context, threadId, playbook.name(), emitProgress, deadline);
                results.add(result);
